...
# 选择性详细播报：传说/神话=完整演出；新发现=🆕 紧凑(名·稀有·尺寸·价值 + 一句描写/手感 + 奖励内联)；
# 稀有/史诗=一行 + 描写；普通/少见=极简一行。描写优先用「手感」(水下鱼有)，否则用图鉴描述。去掉 [实例id]、不重复「图鉴新发现」。
def _format_catch(f, size, value, inst, first):
    u = f["size_unit"]; r = f["rarity"]; rl = RARITY[r]["label"]
    flavor = f.get("capture_feel") or f.get("description", "")
    ...
    if first:   # 🆕 新发现
        return "🆕 %s · %s · %s%s · %d点\n%s（首次收录 +%d点）" % (f["name"], rl, size, u, value, flavor, RARITY[r]["discovery_bonus"])
    if r in ("rare", "epic"):
